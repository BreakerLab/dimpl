...
def build_infernal_commands(data_dir, step_name, no_secondary_structure=False):
    ...
    # Add the outdir variable to the infernal source file
    with open("src/shell/cmfinder_source_template.sh", "r") as cmfinder_source_template:
        data = cmfinder_source_template.read()
    with open("{}/scripts/{}_cmfinder_source.sh".format(data_dir, step_name), "w") as cmfinder_source:
        cmfinder_source.write("STEPNAME={}\n".format(step_name) + data)
    ...
    with open("{}/scripts/{}_infernal_jobfile.sh".format(data_dir, step_name), "w") as jobfile:
        for igr_filename in igr_list:
            ...
            jobfile.write(
                "SEQNAME={}; source scripts/{}_infernal_source.sh; $CMBUILDCOMMAND && $CMCALIBRATECOMMAND && $CMSEARCHCOMMAND\n".format(
                    igr_name, step_name
                )
            )
    ...
    with open("{}/scripts/{}_cmfinder_jobfile.sh".format(data_dir, step_name), "w") as jobfile:
        for igr_filename in igr_list:
            igr_name = igr_filename[:-1].split("/")[-1]
            jobfile.write(
                'SEQNAME={}; source scripts/{}_cmfinder_source.sh; bash -c "$CMFINDERCOMMAND" \n'.format(
                    igr_name, step_name
                )
            )
