...
def main():
    ...
    genomes_query = session.query(Genome).filter(or_(Genome.kingdom=='archaea', Genome.kingdom=='bacteria')).filter(Genome.assembly_acc != '')
    session.close()
...
def download_genome(genome):
    ...
    if not os.path.isfile(genbank_filename):
        ...
        if len(assembly_record_ids) == 1:
            ...
            result = Entrez.read(Entrez.esummary(id=','.join(assembly_query_result['IdList']), db='assembly', ))
            # Get a list of the assembly accessions
            accession_list = [document_summary['AssemblyAccession'] for document_summary in
            ...
            if len(index) != 1:
                raise ValueError("{} records were returned by Entrez when searching for assembly {}"
                                 .format(len(assembly_record_ids), genome.assembly_acc))
            else:
        ...
        for seqrecord in SeqIO.parse(genbank_file, "gb"):
            ...
            for i, position_pair in enumerate(cds_list):
                ...
                if igr_len >= igr_length_cutoff:  # Check if IGR is longer than cutoff.
                    ...
                    igr_list.append({'accession': seqrecord.name,
                                     'start': igr_start,
                                     'end': igr_end,
                                     'length': igr_len,
                                     'gc': igr_gc,
...
def annotate_igrs(genome, igr_df):
    ...
    for accession, accession_igr_df in igr_df.groupby('accession'):
        ...
        try:
            ...
            print("IGR dataframe key: {} not found. Available keys are: {}".format(accession, annotation_tree_dict.keys()  ))
        ...
        for igr in accession_igr_df.itertuples():
            ...
            for overlap in overlap_list:
                ...
                annotated_igr_list.append({'igr_index': igr[0],
                                           'rfam_acc': overlap.value.rfam_acc})

    # Convert annotated_igr_list into dataframe and merge on the rfam_acc
    annotated_igr_df = pd.merge(igr_df, pd.DataFrame(annotated_igr_list, columns=["igr_index", "rfam_acc"]),
                                on="igr_index", how='left')
...
def categorize_igr(row):
    ...
    rfam_id_dict = {"rRNA": "rRNA", "tRNA": "tRNA", "intron": "Intron", "RNaseP": "RNase P",
                    "6S": "6S RNA", "tmRNA": "tmRNA"}
