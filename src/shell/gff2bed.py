      ...
      with openfile(genome_file, 'w') as genome:
        ...
        while not EOF:
            ...
            if not line:
                ...
                (accno,source,gff_type,start,end,score,strand,phase,gff_attributes) = line.rstrip().split('\t')
            ...
            if not first_line and (gff_id not in previous_ids):
                ...
                feature_desc = attribs_saved.get('product',attribs_saved.get('Note',attribs_saved.get('function','')))
...
#                    feature_desc = attribs_saved.get('Note',attribs_saved.get('function',''))
            ...
            previous_data = {'accno':accno, 'start':start, 'end':end, 'type':gff_type, 'score':score, 'strand':strand, 'phase':phase}        
            first_line = False
