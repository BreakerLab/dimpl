...
fix_COMMAND= $'#!/bin/bash\n find . -name \*.tblout | xargs -I {} sh -c "$parent_dir/scripts/fix_tblout_coords.sh {} > {}.tmp && mv {}.tmp {}"; find . -name \*.sto | xargs -I {} sh -c "$parent_dir/scripts/fix_sto_coords.sh {} > {}.tmp && mv {}.tmp {}"'
fix_RESPONSE=$(echo "$fix_COMMAND" | sbatch -p $PARTITION --dependency afterany:${bigmem_JOBID} --job-name fixcoords.$(basename $parent_dir) --chdir $parent_dir/$STEPNAME --output ../output/fixcoords.out)
...
fasta_RESPONSE=$(echo "$fasta_COMMAND" | sbatch -p $PARTITION --dependency afterany:${fix_JOBID} --job-name sto2fasta.$(basename $parent_dir) --chdir $parent_dir/$STEPNAME --output ../output/sto2fasta.out)
fasta_JOBID=${fasta_RESPONSE##* }

# For fasta files with more than 1000 hits, take a random sample of 1000.
sample_RESPONSE=$(sbatch -p $PARTITION --dependency afterany:${fasta_JOBID} --job-name samplefasta.$(basename $parent_dir) --chdir $parent_dir/$STEPNAME --output ../output/samplefasta.out $parent_dir/scripts/dedupe_sample.sh)
