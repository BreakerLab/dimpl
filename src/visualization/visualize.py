...
def graph_layout(genome):
    ...
    layout = go.Layout(
        title=dict(text="<b><i>{}</i></b>".format(genome.scientific_name), x=0.45, y=0.925),
        ...
        xaxis=dict(
            title=dict(text="<b> IGR Length </b>"),
            type="log",
            tickangle=0,
            tickvals=xtickvals,
            ticktext=xticktext,
...
def graph_genome(annotated_df, selection=None):
    ...
    if selection is not None:
        ...
        print("Number of known IGRs included:   {} ({:.1%})".format(knowns_included, knowns_included / total_knowns))
        ...
        point_selection = [
            list(annotated_df[annotated_df["category"] == category].reset_index().query("selection").index)
    ...
    scatter_plots = [
        go.Scatter(
            y=annotated_df[annotated_df["category"] == category]["gc"],
            ...
            marker=dict(size=10, symbol=category_style[category][0], color=category_style[category][1]),
        )
...
def save_selected_IGRs(interactive_plot, annotated_df, genome):
    ...
    with open(
        "{}/data_files/svmclf.pickle".format(output_folder, genome.assembly_acc, class_weight_mod, c_exp, gamma_exp),
        "wb",
    ) as svm_pickle_file:
    ...
    py.io.write_json(
        fig, "{}/data_files/{}_json.plotly".format(output_folder, genome.scientific_name.replace(" ", "_"))
    )
    ...
    selected_igr_list = [
        SeqRecord.SeqRecord(row.sequence, id=("{}/{}-{}".format(row.accession, row.start + 1, row.end)))
    ...
    with tarfile.open(
        "data/export/{}_{}_selection_{}_{}_{}_blastdata.tar.gz".format(
            "_".join(genome.scientific_name.split(" ")[0:2]), genome.assembly_acc, class_weight_mod, c_exp, gamma_exp
        ),
        "w:gz",
    ) as tar:
        tar.add(
            output_folder,
            arcname="{}_{}_selection_{}_{}_{}_blastdata".format(
                "_".join(genome.scientific_name.split(" ")[0:2]),
                genome.assembly_acc,
                class_weight_mod,
                c_exp,
                gamma_exp,
